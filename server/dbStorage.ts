import { type User, type UpsertUser, type Cigar, type InsertCigar, type Release, type InsertRelease, type Event, type InsertEvent, type CommunityPost, type InsertCommunityPost, type PostComment, type InsertPostComment, users, cigars, releases, events, communityPosts, postLikes, postComments } from '@shared/schema';
...
export class DbStorage implements IStorage {
    ...
    try {
      ...
      if (existingReleases.length === 0) {
        await db.insert(releases).values([
          ...
          { name: 'Partagás Serie D No. 5', brand: 'Partagás', releaseDate: new Date('2026-06-10'), region: 'Europe', availability: 'Upcoming', description: 'Full-bodied with dark chocolate and earth notes.' },
          { name: 'H. Upmann Magnum 56 Edición Limitada', brand: 'H. Upmann', releaseDate: new Date('2026-07-01'), region: 'UK & Europe', availability: 'Upcoming', description: 'Cedar, leather and toasted nuts from limited edition programme.' },
          { name: 'Bolivar Royal Coronas', brand: 'Bolivar', releaseDate: new Date('2026-03-10'), region: 'UK', availability: 'Available', description: 'Dark, powerful and complex in a refined robusto format.' },
        ]);
      }
      const existingEvents = await db.select().from(events).limit(1);
      if (existingEvents.length === 0) {
        await db.insert(events).values([
          { name: 'London Cigar Tasting Evening', date: new Date('2026-04-18T19:00:00'), location: 'Davidoff of London, St James\'s', type: 'Tasting', description: 'Exclusive evening of premium Cuban cigars paired with aged rum.', attendees: 12, maxCapacity: 30, link: 'https://davidoff.com' },
          { name: 'Manchester Cigar Social', date: new Date('2026-05-03T18:30:00'), location: 'The Smoking Room, Manchester', type: 'Social', description: 'Monthly meetup for cigar enthusiasts in the North West.', attendees: 8, maxCapacity: 20, link: null },
          { name: 'European Cigar Festival 2026', date: new Date('2026-06-20T10:00:00'), location: 'Amsterdam, Netherlands', type: 'Festival', description: 'Premier cigar festival in Europe. 50+ brands and exclusive releases.', attendees: 320, maxCapacity: 500, link: null },
          { name: 'Habanos Virtual Lounge', date: new Date('2026-04-05T20:00:00'), location: 'Online', type: 'Virtual', description: 'Join master rollers and brand ambassadors for a live online session.', attendees: 45, maxCapacity: 200, link: null },
        ]);
      }
  ...
  async upsertUser(userData: UpsertUser): Promise<User> {
    const result = await db.insert(users).values({ id: userData.id, username: userData.username ?? '', email: userData.email ?? '', password: '' }).onConflictDoUpdate({ target: users.id, set: { username: userData.username ?? '', email: userData.email ?? '' } }).returning();
    return result[0];
  }
  ...
  async rsvpEvent(id: string): Promise<Event | null> {
    ...
    if (event.maxCapacity !== null && (event.attendees ?? 0) >= event.maxCapacity) return null;
  ...
  async toggleLike(postId: string, userId: string): Promise<{ liked: boolean; likes: number }> {
    ...
    if (existing.length > 0) {
      ...
      const result = await db.update(communityPosts).set({ likes: sql`${communityPosts.likes} - 1` }).where(eq(communityPosts.id, postId)).returning();
  ...
  async getComments(postId: string): Promise<PostComment[]> {
    return db.select().from(postComments).where(eq(postComments.postId, postId)).orderBy(postComments.timestamp);
  }
  ...
  async createComment(comment: InsertPostComment): Promise<PostComment> {
    ...
    await db.update(communityPosts).set({ comments: sql`${communityPosts.comments} + 1` }).where(eq(communityPosts.id, comment.postId));
    return result[0];
  }
  ...
  async deleteComment(id: string): Promise<boolean> {
    ...
    if (comment[0]) {
      await db.update(communityPosts).set({ comments: sql`${communityPosts.comments} - 1` }).where(eq(communityPosts.id, comment[0].postId));
    }
